...
var typedword = "";
var revealedWord = "";
var seasonal = { season: "", progress: 0 };
...
function newWord(skipProgress = null) {
  if (seasonal.season) {
    if (!skipProgress) {
      ComfyJS.Say(seasonalWords[seasonal.season].solutions[seasonal.progress]); // show solution message
      seasonal.progress++; // increment seasonal story
      if (seasonal.progress == seasonalWords[seasonal.season].words.length) {// if end of list
        ComfyJS.Say(`Event Complete! ${seasonalWords[seasonal.season].completed}`) // event complete
        setSeason(""); // reset to default game
      }
    }
    selectedWord = seasonalWords[seasonal.season].words[seasonal.progress]
  }
  else {
...
function setSeason(season) {
  seasonal.progress = 0; // reset seasonal progress
  if (seasonalWords[season]) {
    seasonal.season = season;
    ComfyJS.Say(`${seasonal.season.toUpperCase()}: Begin!`)
  }
  else {
    seasonal.season = null; // undefined season reset
    ComfyJS.Say(`Default Word Mode`)
  }
...
ComfyJS.onCommand = (user, command, message, flags, extra) => {
  ...
  if (flags.broadcaster && command === "setseason") {
    setSeason(message.toLowerCase().split(" ")[0]);
  }
